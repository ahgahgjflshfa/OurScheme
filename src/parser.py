...
class Parser:
    ...
    def _parse_list(self) -> ASTNode:
        ...
        while not Parser._is_token_type(self._current_token, "RIGHT_PAREN", "EOF"):
            if Parser._is_token_type(self._current_token, "DOT"):   # 如果遇到 `.`，要轉換成統一的 cons node 結構 e.g. (1 2 . 3) => (1 . (2 . 3))
                if not elements:
                ...
                if Parser._is_token_type(self._current_token, "RIGHT_PAREN"):  # Cons Node 不能沒有 cdr
                    line, pos = self._relative_token_position(self._current_token)
                    value = self._current_token.value
                ...
                token = self._consume_token()
                if Parser._is_token_type(token, "EOF"):   # 沒輸入完
                    # 告訴 parser 和 repl 等待用戶輸入剩下的部分
                    raise NotFinishError("Unexpected EOF while parsing list.")  # 讓 `repl()` 繼續等待輸入
                ...
                return Parser._convert_to_cons(elements, cdr) # 轉換成統一格式
